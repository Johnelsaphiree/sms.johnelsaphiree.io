...
if($save)
{
	...
	if($val->error_code < 1)
	{
		...
		{
			$up = new update();
			$up->up('pages', 'message', 'type', "'pricing'", "'$message'");
		}
		else
		{
			$up = new insert();
			$up->input('pages', 'id, message, type', "0, '$message', 'pricing'");
		}
		$success = success(1);
		header("location: tpricing.php?success=$success");
		exit;
	}
}
...
tinymce.init({
  ...
  if($val->error_code > 0)
	...
	?>
    <form action="tpricing.php" method="post" enctype="multipart/form-data" name="wmsg" class="form-horizontal" role="form">
    <div class="panel panel-default">
  <div class="panel-heading">Pricing Page Instructions</div>
