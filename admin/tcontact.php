...
//get msg
$gm = new select();
$gm->pick('pages', 'message', 'type', "'contact'", '', 'record', '', '', '=', '');
...
if($save)
{
	...
	if($val->error_code < 1)
	{
		...
		{
			$up = new update();
			$up->up('pages', 'message', 'type', "'contact'", "'$message'");
		}
		else
		{
			$up = new insert();
			$up->input('pages', 'id, message, type', "0, '$message', 'contact'");
		}
		$success = success(1);
		header("location: tcontact.php?success=$success");
		exit;
	}
}
...
tinymce.init({
  ...
  if($val->error_code > 0)
	...
	?>
    <form action="tcontact.php" method="post" enctype="multipart/form-data" name="wmsg" class="form-horizontal" role="form">
