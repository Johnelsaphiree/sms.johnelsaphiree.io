...
//get msg
$gm = new select();
$gm->pick('pages', 'message', 'type', "'terms'", '', 'record', '', '', '=', '');
...
if($save)
{
	...
	if($val->error_code < 1)
	{
		...
		{
			$up = new update();
			$up->up('pages', 'message', 'type', "'terms'", "'$message'");
		}
		else
		{
			$up = new insert();
			$up->input('pages', 'id, message, type', "0, '$message', 'terms'");
		}
		$success = success(1);
		header("location: tterms.php?success=$success");
		exit;
	}
}
...
tinymce.init({
  ...
  <li class="active">TERMS OF USE PAGE</li>
</ol>
  ...
  if($val->error_code > 0)
	...
	?>
    <form action="tterms.php" method="post" enctype="multipart/form-data" name="wmsg" class="form-horizontal" role="form">
