...
//get msg
$gm = new select();
$gm->pick('pages', 'message', 'type', "'about'", '', 'record', '', '', '=', '');
...
if($save)
{
	...
	if($val->error_code < 1)
	{
		...
		{
			$up = new update();
			$up->up('pages', 'message', 'type', "'about'", "'$message'");
		}
		else
		{
			$up = new insert();
			$up->input('pages', 'id, message, type', "0, '$message', 'about'");
		}
		$success = success(1);
		header("location: tabout.php?success=$success");
		exit;
	}
}
...
tinymce.init({
  ...
  if($val->error_code > 0)
	...
	?>
    <form action="tabout.php" method="post" enctype="multipart/form-data" name="wmsg" class="form-horizontal" role="form">
