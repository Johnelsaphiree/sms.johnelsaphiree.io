...
//get msg
$gm = new select();
$gm->pick('pages', 'message', 'type', "'reseller'", '', 'record', '', '', '=', '');
...
if($save)
{
	...
	if($val->error_code < 1)
	{
		...
		{
			$up = new update();
			$up->up('pages', 'message', 'type', "'reseller'", "'$message'");
		}
		else
		{
			$up = new insert();
			$up->input('pages', 'id, message, type', "0, '$message', 'reseller'");
		}
		$success = success(1);
		header("location: treseller.php?success=$success");
		exit;
	}
}
...
tinymce.init({
  ...
  <li class="active">RESELLER PAGE</li>
</ol>
  ...
  if($val->error_code > 0)
	...
	?>
    <form action="treseller.php" method="post" enctype="multipart/form-data" name="wmsg" class="form-horizontal" role="form">
